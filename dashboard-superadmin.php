...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_ekskul'])) {
    ...
    $foto_path = $_POST['foto_lama_ekskul'] ?? '';
    ...
    if (isset($_FILES['foto_ekskul']) && $_FILES['foto_ekskul']['error'] === UPLOAD_ERR_OK) {
        ...
        $dest = $uploadDir . $filename;
        if (move_uploaded_file($_FILES['foto_ekskul']['tmp_name'], $dest)) {
            $foto_path = $dest;
            if (!empty($_POST['foto_lama_ekskul']) && file_exists($_POST['foto_lama_ekskul'])) {
                @unlink($_POST["foto_lama_ekskul"]);
            }
        }
    }

    if ($action_ekskul === 'create') {
        $stmt = mysqli_prepare($KONEKSI, "INSERT INTO esktrakulikuler (foto, nama, deskripsi) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "sss", $foto_path, $nama, $deskripsi);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    } elseif ($action_ekskul === 'update' && !empty($_POST['id_ekskul'])) {
        $id = intval($_POST['id_ekskul']);
        $stmt = mysqli_prepare($KONEKSI, "UPDATE esktrakulikuler SET foto=?, nama=?, deskripsi=? WHERE id=?");
        mysqli_stmt_bind_param($stmt, "sssi", $foto_path, $nama, $deskripsi, $id);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    }
...
// NATIVE SERVER CRUD GURU
if (isset($_GET['hapus_guru']) && is_numeric($_GET['hapus_guru'])) {
    $id_hapus = intval($_GET['hapus_guru']);
    $q_foto = mysqli_prepare($KONEKSI, "SELECT foto FROM `nama guru & staff` WHERE id = ?");
    mysqli_stmt_bind_param($q_foto, "i", $id_hapus);
    mysqli_stmt_execute($q_foto);
    $res_foto = mysqli_stmt_get_result($q_foto);
    if ($row_foto = mysqli_fetch_assoc($res_foto)) {
    ...
    $stmt = mysqli_prepare($KONEKSI, "DELETE FROM `nama guru & staff` WHERE id = ?");
    mysqli_stmt_bind_param($stmt, "i", $id_hapus);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    echo "<script>window.location.href='dashboard-superadmin.php?tab=guru&notif_guru=hapus_berhasil';</script>";
    exit();
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_guru'])) {
    $action_guru = $_POST['action_guru'];
    $nama = trim($_POST['nama_guru'] ?? '');
    $mapel = trim($_POST['mapel_guru'] ?? '');
    $foto_path = $_POST['foto_lama_guru'] ?? '';
    ...
    if (isset($_FILES['foto_guru']) && $_FILES['foto_guru']['error'] === UPLOAD_ERR_OK) {
        ...
        $dest = $uploadDir . $filename;
        if (move_uploaded_file($_FILES['foto_guru']['tmp_name'], $dest)) {
            $foto_path = $dest;
            if (!empty($_POST['foto_lama_guru']) && file_exists($_POST['foto_lama_guru'])) {
                @unlink($_POST["foto_lama_guru"]);
            }
        }
    }

    if ($action_guru === 'create') {
        $stmt = mysqli_prepare($KONEKSI, "INSERT INTO `nama guru & staff` (foto, `nama guru`, `mapel guru`) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "sss", $foto_path, $nama, $mapel);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    } elseif ($action_guru === 'update' && !empty($_POST['id_guru'])) {
        $id = intval($_POST['id_guru']);
        $stmt = mysqli_prepare($KONEKSI, "UPDATE `nama guru & staff` SET foto=?, `nama guru`=?, `mapel guru`=? WHERE id=?");
        mysqli_stmt_bind_param($stmt, "sssi", $foto_path, $nama, $mapel, $id);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    }
    $msg = ($action_guru === 'create') ? 'tambah_berhasil' : 'edit_berhasil';
    echo "<script>window.location.href='dashboard-superadmin.php?tab=guru&notif_guru=$msg';</script>";
    exit();
}

$res_guru_query = mysqli_query($KONEKSI, "SELECT * FROM `nama guru & staff` ORDER BY id DESC");
$daftar_guru = [];
while ($row = mysqli_fetch_assoc($res_guru_query)) {
    $daftar_guru[] = $row;
}



// NATIVE SERVER CRUD FASILITAS
if (isset($_GET['hapus_fasilitas']) && is_numeric($_GET['hapus_fasilitas'])) {
    $id_hapus = intval($_GET['hapus_fasilitas']);
    $q_foto = mysqli_prepare($KONEKSI, "SELECT foto FROM `fasilitas` WHERE id = ?");
    mysqli_stmt_bind_param($q_foto, "i", $id_hapus);
    mysqli_stmt_execute($q_foto);
    $res_foto = mysqli_stmt_get_result($q_foto);
    if ($row_foto = mysqli_fetch_assoc($res_foto)) {
    ...
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    echo "<script>window.location.href='dashboard-superadmin.php?tab=fasilitas&notif_fasilitas=hapus_berhasil';</script>";
    exit();
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_fasilitas'])) {
    $action_fasilitas = $_POST['action_fasilitas'];
    $nama = trim($_POST['nama_fasilitas'] ?? '');
    $deskripsi = trim($_POST['deskripsi_fasilitas'] ?? '');
    $foto_path = $_POST['foto_lama_fasilitas'] ?? '';
    ...
    if (isset($_FILES['foto_fasilitas']) && $_FILES['foto_fasilitas']['error'] === UPLOAD_ERR_OK) {
        ...
        $dest = $uploadDir . $filename;
        if (move_uploaded_file($_FILES['foto_fasilitas']['tmp_name'], $dest)) {
            $foto_path = $dest;
            if (!empty($_POST['foto_lama_fasilitas']) && file_exists($_POST['foto_lama_fasilitas'])) {
                @unlink($_POST["foto_lama_fasilitas"]);
            }
        }
    }

    if ($action_fasilitas === 'create') {
        $stmt = mysqli_prepare($KONEKSI, "INSERT INTO `fasilitas` (foto, `nama fasilitas`, deskripsi) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "sss", $foto_path, $nama, $deskripsi);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    } elseif ($action_fasilitas === 'update' && !empty($_POST['id_fasilitas'])) {
        $id = intval($_POST['id_fasilitas']);
        $stmt = mysqli_prepare($KONEKSI, "UPDATE `fasilitas` SET foto=?, `nama fasilitas`=?, deskripsi=? WHERE id=?");
        mysqli_stmt_bind_param($stmt, "sssi", $foto_path, $nama, $deskripsi, $id);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    }
    $msg = ($action_fasilitas === 'create') ? 'tambah_berhasil' : 'edit_berhasil';
    echo "<script>window.location.href='dashboard-superadmin.php?tab=fasilitas&notif_fasilitas=$msg';</script>";
    exit();
}
        ...
        <aside class="sidebar">
            <div class="sidebar-menu">
                ...
                <a href="#" class="menu-item" onclick="showTab('guru')">
                    ...
                    <span>Kelola Guru & Staff</span>
                </a>
                <a href="#" class="menu-item" onclick="showTab('fasilitas')">
                    <i class="icon bx bx-building-house"></i>
            ...
            <div id="content" class="tab-content">
                ...
                <div class="content-grid">
                    ...
                    <div class="content-card">
                        ...
                        <button class="btn-primary" onclick="showTab('guru')">Edit Guru</button>
                    </div>
                    <div class="content-card">
                        <h3>Fasilitas</h3>
                        <p>Kelola data fasilitas sekolah</p>
                        <button class="btn-primary" onclick="showTab('fasilitas')">Edit Fasilitas</button>
                    </div>
                    <div class="content-card">
...
<!-- ===================== KELOLA GALERI TAB ===================== -->
            ...
            <div id="fasilitas" class="tab-content">
                ...
                <div id="formFasilitasWrapNative" style="display:none; margin-bottom: 2rem;">
                    <div class="section-card" style="border-left:4px solid #3b82f6;">
                        <h2 id="formFasilitasTitleNative" style="margin-bottom:1.5rem;">Tambah Fasilitas Baru</h2>
                        <form action="dashboard-superadmin.php" method="POST" enctype="multipart/form-data">
            ...
            <div id="guru" class="tab-content">
                ...
                <div id="formGuruWrapNative" style="display:none; margin-bottom: 2rem;">
                    <div class="section-card" style="border-left:4px solid #8b5cf6;">
                        <h2 id="formGuruTitleNative" style="margin-bottom:1.5rem;">Tambah Guru Baru</h2>
                        <form action="dashboard-superadmin.php" method="POST" enctype="multipart/form-data">
...
// ======== GALERI CRUD NATIVE ========
        ...
        function bukaFormEkskulNative(mode) {
            ...
            if(mode === 'tambah') {
                ...
                document.getElementById('ekskulFotoLamaNative').value = '';
        ...
        function bukaFormFasilitasNative(mode) {
            ...
            if(mode === 'tambah') {
                ...
                document.getElementById('fasilitasFotoLamaNative').value = '';
                document.getElementById('fasilitasNamaNative').value = '';
                document.getElementById('fasilitasDeskripsiNative').value = '';
                document.getElementById('infoFotoLamaFasilitas').innerHTML = '';
            }
            document.getElementById('formFasilitasWrapNative').scrollIntoView({ behavior: 'smooth', block: 'start' });
        }

        function editFasilitasNative(f) {
            document.getElementById('formFasilitasWrapNative').style.display = 'block';
            document.getElementById('formFasilitasTitleNative').textContent = 'Edit Fasilitas';
            document.getElementById('fasilitasActionNative').value = 'update';
            document.getElementById('fasilitasIdNative').value = f.id;
        ...
        function bukaFormGuruNative(mode) {
            ...
            if(mode === 'tambah') {
                ...
                document.getElementById('guruFotoLamaNative').value = '';
                document.getElementById('guruNamaNative').value = '';
                document.getElementById('guruMapelNative').value = '';
                document.getElementById('infoFotoLamaGuru').innerHTML = '';
            }
            document.getElementById('formGuruWrapNative').scrollIntoView({ behavior: 'smooth', block: 'start' });
        }

        function editGuruNative(g) {
            document.getElementById('formGuruWrapNative').style.display = 'block';
            document.getElementById('formGuruTitleNative').textContent = 'Edit Guru';
            document.getElementById('guruActionNative').value = 'update';
            document.getElementById('guruIdNative').value = g.id;
